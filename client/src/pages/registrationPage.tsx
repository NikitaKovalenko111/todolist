        ...
        <div className={cn('container', styles.container)}>
            ...
            <Formik
                ...
                onSubmit={async (values, { setSubmitting }) => {         
                    const registrated = await dispatch(registrationAC(values.username, values.password))
                    setSubmitting(false)
                    if (registrated) {
                        const authorized = await dispatch(authorizationAC(registrated.payload.username, registrated.payload.password))
                        if (authorized) {
                            navigate('/')
                        }
                    }
                ...
                {({
                    ...
                    <form onSubmit={handleSubmit}>
                        <Space direction='vertical' size="large" style={{ display: 'flex', flexDirection: "column", alignItems: 'center', width: 500 }} >
                            ...
                            />
                            <Button className={cn(styles.input)} onClick={() => {handleSubmit()}} >Зарегистрироваться</Button>
                        </Space>
                    </form>
                )}
            </Formik>
            <NavLink className={cn(styles.link)} to="/auth/authorization">Авторизация</NavLink>
        </div>
    )
}
